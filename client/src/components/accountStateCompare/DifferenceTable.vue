    ...
    <v-row>
      <v-col>
        <v-data-table
          class="mx-12"
          :headers="headers"
          :items="dataRowsWithElementDetails"
          :search="search"
          multi-sort
...
export default {
  ...
  computed: {
    ...
    dataRowsWithElementDetails(){
      const result = [...this.dataRows]
      result.forEach(x => {
  ...
  methods: {
    getCSVString(){
      ...
      this.dataRowsWithElementDetails.forEach((line) => {
        let newCsvRow = []
    ...
    downloadCsv() {
      ...
      anchor.download = `${this.description}_${timestamp}.csv`;
      anchor.click();
    }
  }
}
